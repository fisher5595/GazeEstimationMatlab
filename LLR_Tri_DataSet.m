...
for SubjectNumber=0:9
    ...
    triplot(TRI,TrainingPositionMatrix(2,:),TrainingPositionMatrix(1,:));

    S=eye(FeatureDimension);
    Errors=double(zeros(NumOfFeaturesInTesting,1));
    for QueryNumber=1:NumOfFeaturesInTesting
        fprintf('Subject[%d]Query[%d]\n',SubjectNumber,QueryNumber);
    ...
    %     AffinityMatrix3=DisplayAffinityMatrix(TrainingFeatureMatrix,0.1199,S);
        index=FindClosedTriangleAndNeighbors( QueryFeature, TrainingFeatureMatrix, TRI, 1 );
        k_knn=size(index,2);
    ...
    disp(AvgError);
    x.x=Errors;
    save([SaveDir,num2str(SubjectNumber),'/','Errors-48_LLR_TRI', '.mat'],'-struct','x');
end
