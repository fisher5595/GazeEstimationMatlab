...
while Scale>=EndingScale
    ...
    for lamda=StartingLambda:Scale:EndingLambda
        ...
        fprintf('\nLambda[%g] AvgError[%g]\n',lamda,AvgError);
        x.x=Errors;
        save([SaveDir,num2str(SubjectNumber),'/','Errors_Sol3_',sprintf('Lambda-%g',lamda),sprintf('_S-%g',StoppingCriterion),'.mat'],'-struct','x');
...
sendmail(myaddress, sprintf('LambdaAndErrors Sol3 Subject[%02d],StoppingCriterion[%g],BestLambda[%6.3f] AvgError[%8.6f]',SubjectNumber,StoppingCriterion,BestLambda,BestAvgError));
